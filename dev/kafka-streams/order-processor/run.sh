...
# Simulates real-time order processing and aggregation

echo "========================================"
echo "BSS Order Processing Stream"
echo "========================================"
echo ""
...
APPLICATION_ID=${APPLICATION_ID:-"bss-order-processor"}
INPUT_TOPIC=${INPUT_TOPIC:-"bss.order.events"}
...
echo ""
echo "Starting Order Processing Stream..."
...
kafka-console-consumer \
  ...
  2>/dev/null | while IFS= read -r line; do
    ...
    if echo "$line" | jq . >/dev/null 2>&1; then
      ...
      ORDER_ID=$(echo "$line" | jq -r '.data.orderId // "unknown"' 2>/dev/null)
      CUSTOMER_ID=$(echo "$line" | jq -r '.data.customerId // "unknown"' 2>/dev/null)
      ORDER_STATUS=$(echo "$line" | jq -r '.data.status // "unknown"' 2>/dev/null)
      ORDER_AMOUNT=$(echo "$line" | jq -r '.data.amount // "0"' 2>/dev/null)
      TIMESTAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
      
      # Generate analytics event
      ANALYTICS_EVENT=$(jq -n \
        --arg type "order.analytics" \
        --ts "$TIMESTAMP" \
        --arg oid "$ORDER_ID" \
        --arg cid "$CUSTOMER_ID" \
        --arg status "$ORDER_STATUS" \
        --arg amount "$ORDER_AMOUNT" \
        '{
          specversion: "1.0",
          type: $type,
